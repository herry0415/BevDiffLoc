...
writer = SummaryWriter(log_dir='./runs/10_02')
...
def train_fn(cfg: DictConfig):
    ...
    OmegaConf.set_struct(cfg, False) # 运行动态修改参数
    ...
    model, dataloader, optimizer, lr_scheduler = accelerator.prepare(model, dataloader, optimizer, lr_scheduler) #! accelerator
...
def _train_or_eval_fn(model, criterion, dataloader, cfg, optimizer, stats, accelerator, lr_scheduler, epoch, training=True):
    ...
    pose_stats = os.path.join(cfg.train.dataroot, cfg.train.sequence, cfg.train.sequence + '_radar_pose_stats.txt') #! 有修改
