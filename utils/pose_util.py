...
def process_poses(poses_in, mean_t, std_t, align_R, align_t, align_s): #! 把原始 INS/LiDAR 位姿进行对齐、标准化，转换成训练可用的 6DoF 位姿向量
    ...
    for i in range(len(poses_out)):
        ...
        poses_out[i, :3] = align_s * np.dot(align_R, t[:, np.newaxis]).squeeze() # 减去对齐平移 align_t   乘上对齐旋转 align_R  再乘上尺度因子 align_s
    ...
    # max and min
    pose_max = np.max(poses_out[:, :2], axis=0)  # (2,) 所有帧的 x、y 平面坐标范围
